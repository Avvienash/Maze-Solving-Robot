...
#if defined(CYDEV_BOOTLOADER_IO_COMP) && (0u != ((CYDEV_BOOTLOADER_IO_COMP == CyBtldr_SPI_S) || \
                                          (CYDEV_BOOTLOADER_IO_COMP == CyBtldr_Custom_Interface)))


/*******************************************************************************
* Function Name: SPI_S_CyBtldrCommStart
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: SPI_S_CyBtldrCommStop
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: SPI_S_CyBtldrCommReset
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: SPI_S_CyBtldrCommWrite
********************************************************************************
*
* Summary:
         ...
         
{
    ...
    (void) SPI_S_ReadTxStatus();

    /* Write TX data using blocking function */
    #if(0u != SPI_S_MODE_USE_ZERO)
        SPI_S_WriteTxDataZero(pData[buf_index]);
        buf_index++;
    #endif /* (SPI_S_MODE_USE_ZERO == 1u) */

    /* Write TX data using blocking function */
    do
    {
        SPI_S_WriteTxData(pData[buf_index]);
        buf_index++;
    }while(buf_index < size);

    while(0u != timeoutMs)
    {
        if(0u != (SPI_S_ReadTxStatus() & SPI_S_STS_SPI_DONE))
        {
            /* Return success code */
            *count = size;
...
/*******************************************************************************
* Function Name: SPI_S_CyBtldrCommRead
********************************************************************************
*
* Summary:
         ...
         
{
        ...
        */
        if(0u != SPI_S_GetRxBufferSize())
        {
            ...
            */
            do
            {
                oldDataCount = SPI_S_GetRxBufferSize();
                CyDelay(SPI_S_BYTE2BYTE_TIME_OUT);
            }while(SPI_S_GetRxBufferSize() > oldDataCount);
            status = CYRET_SUCCESS;
            break;
        }
    ...
    *count = 0u;
    ...
    while(SPI_S_GetRxBufferSize() > 0u)
    {
        tempCount = SPI_S_GetRxBufferSize();
        *count  =(*count) + tempCount;
        ...
        if(*count < size)
        {
            for (iCntr = 0u; iCntr < tempCount; iCntr++)
            {
                ...
                pData[dataIndexCntr] = SPI_S_ReadRxData();
                dataIndexCntr++;
            }
...
#endif /* if defined(CYDEV_BOOTLOADER_IO_COMP) && (0u != ((CYDEV_BOOTLOADER_IO_COMP == CyBtldr_SPI_S) || \
                                          (CYDEV_BOOTLOADER_IO_COMP == CyBtldr_Custom_Interface)) */
